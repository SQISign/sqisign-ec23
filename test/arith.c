...
proj random_point(proj const *A, long ell, long e) {
    ...
    if (curve_order_is_p_plus_one)
      ...
      uintbig_sub3(&cofactor, &p, &uintbig_1);
    uintbig ell_big;
...
int test_random() {
    ...
    long len = on_curve_len;
    proj P;

    t = tic();
    for (int i = 0; i < repetitions; i++) {
        long ell = on_curve_fact[i % len], e = on_curve_mult[i % len];
...
int test_weil() {
	...
	for (int i = 1; i < e; i++)
	  uintbig_mul3_64(&ellbig, &ellbig, ell);
