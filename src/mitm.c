...
bool MITM_cutoff(two_walk *phi, const proj *from, const proj *to, long len, long tab_size) {
  ...
  // Compute bases  
  find_basis_2e(&Pf, &Qf, &PQf, from);
  find_basis_2e(&Pt, &Qt, &PQt, to);
...
bool MITM2(two_walk *eta, const proj *from, const proj *to, long length) {
  ...
  find_basis_2e(&P3, &P1, &P2, from); // P3 has the (0,0) direction
  ...
  find_basis_2e(&Q3_long, &Q1_long, &Q2_long, to); // P3 has the (0,0) direction
