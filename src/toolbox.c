...
//select the small possible number above the bound
GEN famat_smallest(GEN f1, GEN B) {
  pari_sp ltop = avma;

  int lg_f1 = lg(gel(f1,1)) - 1;
  int m;
  // int big_factors = 2; //accounts for the big factors that we want to try to avoid if possible
...
//big_factors identify the number of big factors that we will a priori try to avoid
//also assume the big factors have valuation 1
GEN famat_random(GEN f1, GEN B, int big_factors) {
    ...
    m = index;
    //we do the loop first without the big factors
    while (sum < logB && total_available > 2*big_factors) {
        n = random_Fl(total_available-2*big_factors);
        index = -1;
        acc = 0;
        while (acc <= n) {
            index++;
    ...
    //then we do it a second time to complete with the big factors if needed
    while (sum < logB && total_available > 0) {
...
// same as above
// when in_full_max = false, ensures that (x+v) and (y+u) are both even
// (this means that x + y*i + u*j + v*ji is in 2*Order(1,i,(1-ji)/2, (i+j)/2)
    ...
    while (count < 10000 ) {
        ...
        if (randomized) {
            ...
            if (upper_bound == 0) {
              ...
              u = random_Fl((long)sqrt(2)*bound_randomized);
              long new_up = (long)(sqrt( upper_bound - u*u ));
              v = random_Fl(new_up);
            }
        ...
        if (cornacchia_extended(N, &x, &y)) { // no bad small factor
            if ( (in_full_max) ||  ( (smodis(gaddgs(x,v),2) == 0) && (smodis(gaddgs(y,u),2) == 0) ) ) {
                GEN res = mkvec4(x,y,stoi(u),stoi(v));
                return gerepilecopy(ltop,res);
            }
            // else {
            //   GEN res = mkvec4(x,y,stoi(v),stoi(u));
            //   return gerepilecopy(ltop,res);
            // }
        }
